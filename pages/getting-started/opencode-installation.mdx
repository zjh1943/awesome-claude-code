...
A:
- **方法一**：检查配置文件中的 `baseURL` 是否正确，确保以 `/v1` 结尾
- **方法二**：检查 CC Switch 配置的 Base URL 是否正确
- **方法三**：检查 `models` 配置是否正确，确保模型 ID 配置正确
...
1. 网络连接是否正常
2. API Key 是否正确
3. Base URL 是否正确（必须以 `/v1` 结尾）
4. 如使用方法三，检查 New API 服务是否正常运行

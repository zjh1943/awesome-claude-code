...
<Callout type="warning">
  请注意：一定要重启 PowerShell，一定要重启 PowerShell！这次不要用管理员身份，就正常打开就行！
</Callout>
...
**原因**：Node.js 版本过低。Claude Code 需要 Node.js 18.x 或更高版本。

**解决方法**：

1. 检查当前版本：`node --version`
2. 如果版本低于 v18，请升级 Node.js：
   - 访问 https://nodejs.org/ 下载最新 LTS 版本
   - 或使用 winget：`winget install OpenJS.NodeJS`
3. 升级后重新打开 PowerShell 并验证：`node --version`
4. 重新运行安装脚本或 `npm install -g @anthropic-ai/claude-code`

详细说明请参考：[SyntaxError 解决方案](/getting-started/common-errors/syntax-error-node-version)

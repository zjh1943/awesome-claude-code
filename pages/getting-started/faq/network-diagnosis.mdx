...
如果按照上述方法测试后仍无法解决问题，可以使用 Claude Code 的 debug 模式收集详细日志，发送给管理员进行深度分析。
...
收集完成后，日志会保存到 `/tmp/claude-network-debug.log`

```bash
# 查看日志内容
cat /tmp/claude-network-debug.log

# 将日志发送给管理员（可以复制内容或发送文件）
```
  </Tabs.Tab>
  <Tabs.Tab>
```powershell
...
# 日志文件位置
Write-Host "日志文件: $env:TEMP\claude-network-debug.log"
```
  </Tabs.Tab>
</Tabs>
...
|-----|------|
| `--debug-file` | 指定日志保存路径 |
| `--debug "api,mcp"` | 开启 API 和 MCP 相关的详细调试信息 |
| `--model claude-haiku-4-5-20251001` | 使用最快的模型减少等待时间 |
| `-p "test network connection"` | 发送简单测试消息 |
...
<Callout type="warning">
debug 日志可能包含敏感信息（如 API 密钥片段、请求路径等），发送给管理员前请确认日志内容，或使用 CC Club 的[工单系统](https://claude-code.club)安全上传。
</Callout>

<Callout type="info">
使用 Haiku 模型是为了最小化 token 消耗和等待时间，同时生成完整的网络请求日志。这个测试只会消耗极少量 token。
</Callout>
